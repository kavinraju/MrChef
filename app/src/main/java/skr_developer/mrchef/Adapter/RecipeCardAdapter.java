...
public class RecipeCardAdapter extends RecyclerView.Adapter<RecipeCardAdapter.RecipeCardViewHolder>{
    ...
    //private int[] recipeDrawables = {R.drawable.nutella_pie, R.drawable.brownie, R.drawable.yellow_cake, R.drawable.cheesecake};
        ...
        if (position< recipeList.size()){
            switch (recipeList.get(position).getName()){
                case "Nutella Pie":
                    ...
                    helperClass.loadImage(context, helperClass.getRecipe_url(0), recipeCardViewHolder.iv_recipeCard);
                    break;
                case "Brownies":
                    //recipeCardViewHolder.iv_recipeCard.setBackgroundResource(recipeDrawables[1]);
                    helperClass.loadImage(context, helperClass.getRecipe_url(1), recipeCardViewHolder.iv_recipeCard);
                    break;
                case "Yellow Cake":
                    //recipeCardViewHolder.iv_recipeCard.setBackgroundResource(recipeDrawables[2]);
                    helperClass.loadImage(context, helperClass.getRecipe_url(2), recipeCardViewHolder.iv_recipeCard);
                    break;
                case "Cheesecake":
                    //recipeCardViewHolder.iv_recipeCard.setBackgroundResource(recipeDrawables[3]);
                    helperClass.loadImage(context, helperClass.getRecipe_url(3), recipeCardViewHolder.iv_recipeCard);
                    break;
            }
